...
void MMult1(long m, long n, long k, double *a, double *b, double *c) {
  ...
  for (long jj = 0; jj < n; jj+=BLOCK_SIZE) {
	...
	for (long j = jj; j < fmin(n, jj+BLOCK_SIZE); j++){
	  for (long p = pp; p < fmin(k, pp+BLOCK_SIZE); p++){
	    for (long i = ii; i < fmin(m, ii+BLOCK_SIZE); i++){
	      double A_ip = a[i+p*m];
	      double B_pj = b[p+j*k];
	      double C_ij = c[i+j*m];
	      C_ij = C_ij + A_ip * B_pj;
...
int main(int argc, char** argv) {
  ...
  for (long p = PFIRST; p < PLAST; p += PINC) {
    ...
    double bandwidth = NREPEATS*(p*p)*(2*p/BLOCK_SIZE+2)*sizeof(double)/time/1e9; // TODO: calculate from m, n, k, NREPEATS, time
