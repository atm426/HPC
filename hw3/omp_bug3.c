...
void print_results(float array[N], int tid, int section) 
{
  ...
  //#pragma omp barrier
  // we didn't need this barrier, i think it causes the program to hang
  // not all threads run the "print_results" function so the barrier may be waiting for threads that will never arrive
  // on my laptop the program doesn't hang but I think this is the intended solution
